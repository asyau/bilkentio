...
const PuantajScores = () => {
        ...
        const guidesWithStats = await Promise.all(
          response.data.map(async (guide) => {
            const statsResponse = await axios.get(
              `http://localhost:8080/api/guides/${guide.id}/stats`,
              {
                headers: {
  ...
  const getLastMonthTours = (tours) => {
    ...
    return tours.filter((tour) => {
      const tourDate = new Date(tour.date);
      return (
        tourDate.getMonth() === lastMonth.getMonth() &&
  ...
  const fetchGuideDetails = async (guideId) => {
    try {
      ...
      const [
        ...
        reviews,
        completedGroupTours,
        completedIndividualTours,
        upcomingTours,
      ] = await Promise.all([
        axios.get(`http://localhost:8080/api/guides/${guideId}/stats`, {
          headers,
        }),
        axios.get(`http://localhost:8080/api/guides/${guideId}/reviews`, {
          headers,
        }),
        axios.get(
          `http://localhost:8080/api/guides/${guideId}/tours/completed`,
          { headers }
        ),
        axios.get(
          `http://localhost:8080/api/individual-tours/guide/${guideId}`,
          { headers }
        ),
        axios.get(
          `http://localhost:8080/api/guides/${guideId}/tours/upcoming`,
          { headers }
        ),
      ]);
      console.log(completedGroupTours.data);
  ...
  const renderTourCard = (tour, type = "upcoming") => (
    <div key={tour.id} className={`tour-card ${type}`}>
      ...
      <div className="tour-info">
        <p>
          <span className="material-icons">location_on</span> {tour.city}
        </p>
        {tour.schoolName && (
          <p>
            <span className="material-icons">school</span> {tour.schoolName}
          </p>
        )}
        {tour.groupSize && (
          <p>
            <span className="material-icons">group</span> {tour.groupSize}{" "}
            people
          </p>
        )}
        {tour.interests && (
          <p>
            <span className="material-icons">interests</span> {tour.interests}
          </p>
        )}
        {tour.totalHours && (
          <p>
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case "completed":
        return (
          <div className="tours-grid">
            {guideDetails.tours.completed.map((tour) =>
            ...
            {guideDetails.reviews.map((review, index) => (
              <div key={index} className="review-card">
                <div className="review-header">
                  ...
                  {review.tourType && (
                    <div className="review-tour-type">{review.tourType}</div>
                  )}
                </div>
                    ...
                    <div className="metric">
                      ...
                      <div className="metric-details">
                        ...
                        <span className="value">
                          {selectedGuide.averageRating?.toFixed(1) || "-"} ★
                        </span>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="stats-grid">
                      ...
                      <div className="stat-card">
                        ...
                        <div className="stat-info">
                          ...
                          <p>
                            {guideDetails.stats.currentMonthHours.toFixed(1)}
                          </p>
                        </div>
                      </div>
                    </div>
